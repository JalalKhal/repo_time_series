...
-MA(2) d'après l'ACF (après une différenciation de la série originale). (a)
-AR(5) d'après le PACF (après une différenciation de la série originale). (b)
-ARIMA(p=5,d=1,q=2).
On considère ARIMA(5,1,2) car (a) puis (b).
```{r}
arima_512<-arima(ht_cnt$fitted[,'xhat'],order = c(5, 1, 2))
fitted_arima_512 <-ht_cnt$fitted[,'xhat']-arima_512$residuals
plot(fitted_arima_512,col="red")
lines(ht_cnt$fitted[,'xhat'])
arima_512
```
en rouge: modèle ARIMA(5,1,2) pour la série temporelle smoothed.
en noir: smoothed time series.

```{r}
Box.test(arima_512$residuals,lag=20,type="Ljung")
```
On voit que la p-value ~0.9992 pour Box-Ljung test est très proche de 1 (>0.05) donc les 20 premiers résidus sont indépendants.(auto corrélation =0 pour les 20 premiers résidus -->acceptation de l'hypothèse nulle du test).
-->résidus valident pour le modèle (bruit blanc).
```{r}
acf(arima_512$residuals,lag.max=20)
pacf(arima_512$residuals,lag.max=20)

```
Pas d'auto-corrélation ni d'auto corrélation partielle pour les résidus (20 premiers) --> résidu est bien un bruit blanc.

```{r}
plot(arima_512$residuals)
sprintf("Moyenne des résidus:%f et Variance des résidus: %f",round(mean(arima_512$residuals),digits=2),round(var(arima_512$residuals),digits=2))
```
...
AIC=6087.39 pour ARIMA(0,1,2) / AIC=6088.82 pour ARIMA(5,1,2) / AIC=6097.87 pour ARIMA(5,1,0).
ARIMA(0,1,2) est le meilleure modèle car c'est le plus simple (moins de paramètres -->prévenir l'overfitting) et le plus
petit AIC.
...
diff_deseason_cnt=diff(deseason_cnt,differences=1)
...
-AR(5) d'après le PACF (après une différenciation de la série originale). (b)
-ARIMA(p=5,d=1,q=2).
On considère dans un premiers temps ARIMA(5,1,2) car (a) puis (b).

```{r}
arima_cnt_512<-arima(deseason_cnt,order = c(5, 1,2))
fitted_arima_cnt_512 <-deseason_cnt-arima_cnt_512$residuals
plot(fitted_arima_cnt_512,col="red")
lines(deseason_cnt)
arima_cnt_512
```

```{r}
Box.test(arima_cnt_512$residuals,lag=20,type="Ljung")
```

À un seuil de risque de 5%, p-value ~0.018 donc on rejete l'hypothèse d'indépendance des 20 premiers résidus.
...
```{r}
plot(arima_cnt_512$residuals)
sprintf("Moyenne des résidus:%f et Variance des résidus: %f",round(mean(arima_cnt_512$residuals),digits=2),round(var(arima_cnt_512$residuals),digits=2))
```
...
On remarque que la log-vraisemblance dans les trois modèles est autour de -6000 sans différence significative.
Ainsi que l'AIC pour les trois modèles à environ 12000.
Les trois modèles engendrent des résidus non-indépendants.
Nous choisissons donc le modèle le moins complexe pour prévenir l'overfitting ARIMA(0,1,2).
...
On remarque tout d'abord, les résidus ne sont pas indépendants, ce qui s'explique:
La time series se sépare entre trois parties:
avant milieu 2011 et après milieu 2012 --> time series stationnaire sans tendance.
Entre les deux période: une tendance linéaire (droite).
